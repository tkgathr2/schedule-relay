'use client';
/**
 * /admin/relay 一覧テーブル。PATCH /api/admin/relay-links/[slug] で有効/無効を切替。
 */
...
export default function RelayTable() {
  ...
  async function load() {
    try {
      const res = await fetch('/api/admin/relay-links', { cache: 'no-store' });
  ...
  async function toggle(slug: string, next: 'active' | 'disabled') {
    setBusy(slug);
    try {
      const res = await fetch(`/api/admin/relay-links/${slug}`, {
        method: 'PATCH',
  ...
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff' }}>
      ...
      <tbody>
        {rows.length === 0 && (
          <tr>
            <td colSpan={8} style={{ padding: 16, color: '#57606a' }}>
              リレーリンクがまだありません。
            </td>
          </tr>
        )}
